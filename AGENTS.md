...
A user now reports two new-firmware badges showing 3 devices after minutes
beside an older C-firmware badge showing 12. No RF root cause is established.
...
retains startup/vectors, the WCH/FOSSASIA drivers, interrupts, nametag storage,
allocator/flash, buttons, and ROM ISP. Older references below to a pending C
policy replacement describe the historical sequence, not the current code.
Exact-image hardware evidence remains separate and has not been inferred from
host emulation or successful builds.

- Build the application with Rust `1.98.1`, IMC target, `llvm-tools-preview`;
  final link stays MRS V1.92. Remove only non-loadable `.riscv.attributes` with
  LLVM objcopy; old WCH objcopy cannot process every modern Rust object.
  Retain all linked instruction/vector/RAM and exact ELF-to-BIN audits.
- `tools/emulator/` runs the same safe core with a wrapping clock and SDK faults.
  `bash scripts/verify-rust-abi` links the real Rust archive to C golden/ABI tests
  and an allocator/flash fault fixture generated by the production patcher.
- Never call Rust's session from an ISR or invoke SDK callbacks while its mutable
  borrow exists. Commit C wake/display state before commands that may reenter.
- Cancel timeout is not proof of radio idle. Keep new scans, advertising, and
  shutdown blocked until completion or `bleIncorrectMode`; also wait for actual
  advertising and peripheral disconnection before the power handoff.
- The patchable config needs exactly one literal magic in the linked image and
  volatile reads at boot. Keep its default constructor inlined; otherwise LTO
  can materialize an extra constant pool copy or substitute unpatched bytes.
- Python HCI unit tests mock Linux ABI constants as well as the socket so they
  run on Python builds compiled without Bluetooth headers.
