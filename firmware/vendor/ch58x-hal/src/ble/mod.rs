...
pub fn init(
    ...
    // The precompiled BLE library writes through this address. Form a raw
    // mutable pointer directly so no shared reference to `static mut` exists.
    cfg.MEMAddr = unsafe {
        core::ptr::addr_of_mut!(BLE_HEAP.0).cast::<MaybeUninit<u8>>() as u32
    };
