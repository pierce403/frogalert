...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; newer candidate **BLOCKED** for hardware | The published hashes retain the confirmed detector/display behavior. Current source additionally builds audited 206,216-byte profile candidates with debounced KEY1 rail detection, adaptive button/wake routing, a one-second visible app cue, Meta-pair detection, and a steady completed-result counter while retaining the KEY2 ISP symbols | The current `260404` SHA is `a3cb7481…bcc444e`; the current `250901` SHA is `cb2780b1…f0706a`. Do not replace the beta until correct-profile and deliberately mismatched hardware tests pass. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, adaptive buttons, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Its visible Bluetooth cue ends after one second while the app window remains open for ten. Locked images are 206,304 bytes | Top (`260404`) SHA is `5c69637a…00ea8ce`; bottom (`250901`) SHA is `5634194f…7964bd`. Verify the correction on both boards, including repeated presses, app connection, alerts, power draw, and KEY2 recovery. |
