...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; physical-position `0.2.0-beta.12` source **BLOCKED** for hardware | The physical bottom button rotates uploaded names with a separately rendered count view and has no pairing side effect on either exact profile. The physical top button cycles normal → Bluetooth download → hardware shutdown; a qualified wake cold-boots normal. Passive scans and temporary alerts run in either visible view, then the selected view is restored | Retest both beta.12 images: bottom-only view/brightness, top-only download/off/wake without USB, charge wake, repeated cycles, screen-off current, and continuous KEY2-to-ISP from dark. |
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; physical-position `0.2.0-beta.12` source **BLOCKED** for new-hash hardware test | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. The physical top button alone shows the Bluetooth animation and enables persistent advertising. The physical bottom button only changes the name/count view and never advertises. Active bitmap streaming retains display ownership | Button roles remain compile-time exact: `260404` uses KEY2 system/KEY1 view; `250901` uses KEY1 system/KEY2 view. Verify upload/streaming after cold wake, disconnect, screen-off radio silence/current, and survey resumption on both. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.12` source **BLOCKED** for hardware | Current source keeps exact physical-position roles and enters `LowPower_Shutdown(0)` only after asynchronous discovery cancellation confirms radio idle. It retains Meta-pair and Flipper `0x3081`–`0x3083` detection, steady completed counts, compact boot/version cards, and measured battery status. Local counter receipts are top `a75116b7…65799` (201,112 bytes) and bottom `fa891411…e5b4` (201,200 bytes) | Canonical CI publishes the standard top/bottom pair after build, audit, attestation, quarantine, and provenance checks. Keep `hardware_verified: false` until both exact images pass current, wake, recovery, USB/App, display, and survey tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, physical-position button roles, boot/version/battery cards, hardware shutdown, and early KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Beta.12 local receipts are top `6850041d…47716` (201,200 bytes) and bottom `b8c74a35…7a5a` (201,284 bytes) | The standard publication path does not release this optional lane; physically test both candidates, including presses, brightness, app connection, alerts, screen-off current, wake, and KEY2 recovery. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.12` and compact `v0.2.0b12`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Physical button behavior with safe screen off | `0.2.0-beta.12` source **BLOCKED** for exact-hash hardware test | Bottom short rotates name/count and bottom long changes brightness; top short cycles `NORMAL → DOWNLOAD → hardware shutdown`. Shutdown waits for radio idle, stops TMR0/TMR3, then powers down BLE, USB, TMOS, and the matrix. Wake cold-boots. `260404` arms physical-top KEY2; `250901` arms physical-top KEY1 plus KEY2 for held ISP recovery. A marked wake qualifies KEY2 before peripheral startup | Test repeated presses, one-minute current, top wake without USB, charge wake, `250901` short KEY2 remaining dark, continuous roughly 2.2-second KEY2-to-ISP on both, and post-wake USB/App/survey behavior. |
| Brightness and power controls | **PROTOTYPE** in beta.12 | Physical-bottom brightness timing is unchanged. Screen off now uses CH582 `LowPower_Shutdown(0)`; reset-keep magic and GPIO handlers cover wake/reset classification before the ordinary application starts | Verify brightness boundaries, normal/download/screen-off current, exact-profile top wake, repeated cycles, and uninterrupted KEY2 recovery from dark. The ultra-fast released `250901` KEY2 source cannot be distinguished from top/charge after reset and remains a hardware acceptance risk. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.12 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app still uploads normally before marking it hardware-verified. |
