...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; strict-role `0.2.0-beta.8` source **BLOCKED** for hardware | Upstream's KEY2 display button rotates uploaded names with a separately rendered count view and has no pairing side effect. KEY1 keeps download mode and brightness, but its next short press returns to normal instead of shutdown. Passive scans and temporary alerts run in either view, then the selected view is restored | Exact beta.7 testing showed the remaining compatibility app window made KEY2 look like the pairing button. Retest both exact beta.8 images, including view-only KEY2, KEY1 download-to-normal return, and KEY2-only ISP. |
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; strict upstream-role `0.2.0-beta.8` source **BLOCKED** for new-hash hardware test | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. KEY1 alone shows the Bluetooth animation and enables persistent advertising until the next KEY1 short press. KEY2 only changes the name/count view and never advertises. Active bitmap streaming retains display ownership | Earlier candidates added an either-button app-attention window for cross-image compatibility. Physical use showed that this made the view button look like the pairing button, so the window and its timers are removed. Reflash both profiles and verify KEY1 enter/exit, KEY2 view-only presses, upload/streaming, disconnect, selected-view restoration, and survey resumption. |
...
| Automatic USB-C profile detection | **REJECTED** | Untouched KEY1 is open on both boards, and a physical `250901` test showed the held-KEY1 weak-pull probe could falsely classify open PA1 as `260404`, swapping the short-button roles. Current source keeps the artifact's compiled KEY1 polarity, button routing, and shutdown wake | Require the exact printed marking and matching profile for every flash. Deliberate cross-profile images remain useful only for proving fail-safe recovery boundaries; firmware must not silently reinterpret their hardware profile. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.8` source **BLOCKED** for hardware | The published legacy hashes retain the confirmed detector/display behavior. Current source keeps exact profile-bound KEY1 polarity/wake and strict upstream logical button roles while preventing entry into unverified shutdown, plus Meta-pair and Flipper `0x3081`–`0x3083` service detection, a steady completed-result counter, compact boot/version cards, and measured battery status | Canonical CI publishes the standard top/bottom pair automatically after build, audit, attestation, quarantine, and provenance checks. It remains `hardware_verified: false` until each exact image passes its matching-board tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, strict upstream button roles, boot/version/battery cards, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. KEY2 changes only the view; KEY1 alone owns Bluetooth download mode. Current images are 200,220 bytes | `0.2.0-beta.8` pre-CI local receipts are top/`260404` `61d8f495…75fb57fe` and bottom/`250901` `33ace6e6…022c0fba`. The standard publication path does not release this optional lane; physically test both ignored local candidates, including repeated presses, app connection, alerts, power draw, and KEY2 recovery. |
...
| Upstream button behavior with safe return | Published `0.2.0-beta.7` behavior still coupled KEY2 to pairing; strict-role `0.2.0-beta.8` source **BLOCKED** for retest | Both profiles keep FOSSASIA's logical roles: KEY1 short enters download and KEY1 long changes brightness; KEY2 short rotates name/count without any radio/display cue side effect and KEY2 long retains the 200 ms, more-than-ten-sample ISP poll. A second KEY1 short returns `DOWNLOAD → NORMAL` instead of entering `POWER_OFF` | Beta.7 fixed the unrecoverable shutdown but retained a compatibility app window on KEY2. Test repeated KEY2 view-only presses, KEY1 advertising enter/exit, return to the selected nametag flow, and a continuous roughly 2.2-second KEY2 hold while the application is running. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.8 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app still uploads normally before marking it hardware-verified. |
...
| Peripheral/observer role switching | **PROTOTYPE** in source, **BLOCKED** for hardware | WCH Central and Peripheral roles initialize together. KEY1 download mode suspends discovery before enabling persistent advertising; KEY2 view selection has no radio side effect. Otherwise advertising is paused before each survey rather than scanning concurrently, and a connection suspends surveys | Repeated 24-hour run plus Android discovery/upload/reconnect tests are required on both exact boards. |
