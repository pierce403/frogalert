...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; physical-position `0.2.0-beta.11` source **BLOCKED** for hardware | The physical bottom button rotates uploaded names with a separately rendered count view and has no pairing side effect on either exact profile. The physical top button cycles normal → Bluetooth download → recoverable screen off → normal. Passive scans and temporary alerts run in either visible view, then the selected view is restored | The owner verified the same physical positions on factory `260404` and `250901` firmware. Retest both beta.11 images: bottom-only short view changes and long brightness, top-only download/off/wake, no USB-assisted recovery, and long-KEY2 ISP from screen off. |
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; physical-position `0.2.0-beta.11` source **BLOCKED** for new-hash hardware test | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. The physical top button alone shows the Bluetooth animation and enables persistent advertising. The physical bottom button only changes the name/count view and never advertises. Active bitmap streaming retains display ownership | Button roles are compile-time board mappings, not runtime compatibility guesses: `260404` uses KEY2 system/KEY1 view; `250901` uses KEY1 system/KEY2 view. The button header imports those profile constants and fails compilation for a missing or unsupported profile. Verify upload/streaming, disconnect, view restoration, screen-off radio silence, and survey resumption on both. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.11` source **BLOCKED** for hardware | The published legacy hashes retain the confirmed detector/display behavior. Current source keeps exact profile-bound KEY1 polarity and factory-confirmed physical button-position roles, uses a recoverable application screen-off state instead of CH58x shutdown, and retains Meta-pair and Flipper `0x3081`–`0x3083` detection, steady completed counts, compact boot/version cards, and measured battery status | Canonical CI publishes the standard top/bottom pair automatically after build, audit, attestation, quarantine, and provenance checks. It remains `hardware_verified: false` until each exact image passes its matching-board tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, physical-position button roles, boot/version/battery cards, recoverable screen off, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Current images are top 200,416 bytes and bottom 200,448 bytes | `0.2.0-beta.11` pre-CI local receipts are top/`260404` `ca33c4a9…c4b1bc7` and bottom/`250901` `48a48978…3877d4c`. The standard publication path does not release this optional lane; physically test both ignored local candidates, including repeated presses, brightness, app connection, alerts, screen-off current draw, wake, and KEY2 recovery. |
...
| Physical button behavior with safe screen off | `0.2.0-beta.11` source **BLOCKED** for exact-hash hardware test | Both profiles expose the factory-confirmed positions: bottom short rotates name/count only and bottom long changes brightness; top short cycles `NORMAL → DOWNLOAD → screen off → NORMAL`. The screen-off state disables advertising and surveys, clears the framebuffer, disables the 16 kHz TMR0 display interrupt/timer, and releases all matrix pins, but deliberately leaves TMR3 button scanning, TMOS, USB, and the 200 ms long-KEY2 ISP poll alive. `260404` compiles KEY1 bottom/KEY2 top; `250901` compiles KEY2 bottom/KEY1 top | Test repeated short and long position-based presses, at least one-minute screen off, top-button wake without USB, screen-off current draw, and a continuous roughly 2.2-second KEY2 hold from screen off on both exact boards. |
| Brightness and power controls | **PROTOTYPE** in beta.11 | A physical-bottom hold changes brightness on both profiles. Top/`260404` keeps upstream KEY1 activation after 25 samples, about 0.5 seconds. Bottom/`250901` queues KEY2 brightness only on release after 25 through 99 samples, about 0.5 through just under 2 seconds, leaving a continued hold for the unchanged roughly 2.2-second ISP path. Physical top short reaches a recoverable display/radio-off state that can wake without USB | Verify a short bottom press never changes brightness, a brightness hold never emits the short view action, a long top hold never changes brightness, and uninterrupted bottom KEY2 still reaches ISP on `250901`. Measure current in normal, download, and screen-off states. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.11 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app still uploads normally before marking it hardware-verified. |
