...
| Dedicated mobile-first `/flash/` wizard | **SHIPPED** at static UI/browser-test layer | Exactly one pane is visible: detect normal `0416:5020` application mode and guide KEY2/dot entry or read-only identify ISP, choose exact board/BIN, confirm, flash/verify, then terminal success. Previously authorized application or ISP devices are detected automatically; first-time WebUSB permission remains one explicit chooser tap. The old dashboard is hidden from both display and accessibility trees. | `0416:5020` is a mode hint, not hardware or firmware proof. Android Chrome with USB OTG remains physically unverified; hardware access is experimental. |
...
| Detect connected normal-mode badge | **PROTOTYPE** at code/test layer | The chooser and authorized-device scan recognize application USB `0416:5020`, keep the wizard on step 1, then try the nearest and farthest buttons in order. A reported dot suggests `250901` or `260404` respectively, but the next step still requires the exact printed marking. Application interfaces are not opened or claimed. | Browser USB permission is still required once; the shared application id and self-reported button result cannot prove exact hardware or firmware. Neither button working reaches the expert-recovery stop boundary; C3 is not a public checklist. Physical browser test pending. |
