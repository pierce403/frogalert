...
| FOSSASIA-shell passive survey candidates | **PROTOTYPE** at local build layer, **PARTIAL** on `260404` hardware | Both profiles build separately with embedded profile/config ids, passive start/cancel/suspend symbols, bounded configurable OUI/name/service classifier, exact view rotation, fixed count/page alert renderers, queued-animation ownership guards, qualified 48-column frame cropping, live-result and completion-list paths, live vectors, retained USB/BLE/display/KEY2 symbols, no AMO/LR/SC, ELF/BIN identity, and RAM headroom audit. The user confirmed runtime, display, BadgeMagic upload, button behavior, and KEY2 dot-to-ISP recovery for exact `260404` SHA `c6d06c59…face0d9` | CI bundles both profiles in an expiring Actions artifact. The `250901` image remains hardware-unverified. The tested `260404` image still lacks captured exact-hash CLI and WebUSB program/byte-verify transcripts, so it is downloadable for developers but not manifest/release approved. |
...
| Commit-bound CI candidate bundle | **PROTOTYPE** for dual-profile schema v2; current dual-profile lane live-CI verified | CI run `30411963327` built both locked profiles for commit `e8787b8`, packaged deterministic `0.0.0-candidate.e8787b850cc6` BIN/ELF pairs, checksums, and JSON as `frogalert-candidate-<full-commit>` | The front page links the exact expiring Actions artifact for developer download. It requires GitHub sign-in, expires August 28, 2026, declares every publish/approval flag false, and remains outside the manifest/GitHub Release/Pages executable catalog. |
...
| Front-page firmware download | **SHIPPED** for current developer candidate | Hero links the exact successful `e8787b8` Actions artifact and names the tested `260404` BIN/hash | GitHub sign-in is required and the bundle expires August 28, 2026. This does not make either profile a manifest-approved release or permit automatic browser selection. |
...
| Dedicated mobile-first `/flash/` wizard | **SHIPPED** at static UI/browser-test layer | Exactly one pane is visible: detect normal `0416:5020` application mode, guide bottom/top button and dot entry, read-only identify ISP, automatically fetch and validate the approved button-matched image, confirm, flash/verify, then terminal success. No file, profile, or marking control appears in the public wizard. Previously authorized devices are detected automatically; first-time WebUSB permission remains one explicit chooser tap. The old dashboard is hidden from both display and accessibility trees. | The empty approved catalog currently produces one clear stop message rather than exposing a developer-file fallback. `0416:5020` remains a mode hint, not hardware or firmware proof. Android Chrome with USB OTG remains physically unverified. |
...
| Automatic approved image selection | **SHIPPED** at UI/state-test layer; **BLOCKED** for an actual public update | After read-only ISP identification, the guided button result fills the internal exact profile/marking, loads the newest matching schema-v4 release from the same origin, and rechecks descriptor, byte length, SHA-256, embedded profile, quarantine status, and evidence gates. The wizard advances without a file chooser only when all checks pass. | The current manifest has no approved USB-C release, so both button paths stop safely. Entering ISP before the wizard observes a button also stops; there is no guessed profile or local-file fallback. |
| Pre-armed immediate update after ISP appears | **BLOCKED** | Intended flow is to select an approved profile image and explicitly arm the destructive operation before ISP entry, then identify and program without spending the ROM window on more UI. | The public manifest currently has no approved `250901` or `260404` FrogAlert release. Device detection alone must never erase; an exact hosted image and explicit final authorization are required first. |
...
| Passive USB-C KEY1 wiring/polarity profile detection | **REJECTED** for safe flashing | Docs explain why an untouched open KEY1 cannot distinguish pull-up/active-low `260404` from pull-down/active-high `250901`; CHIP ID and USB descriptors also cannot distinguish them | The simple wizard uses the observed profile-specific KEY2 button path instead. If it did not observe that path, it refuses automatic image selection. |
...
| Local `.bin` file selection | **PROTOTYPE**, developer lab only | File never uploads; base hash, final hash, and bound profile are shown locally | Hidden from the public `/flash/` wizard. Developer artifacts never substitute for a missing approved release. |
