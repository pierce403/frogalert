...
| Dedicated mobile-first `/flash/` workflow | **SHIPPED** at static UI/test layer | Compact phone-first path puts WebUSB and local BIN selection first, keeps step navigation and the final action visible on narrow screens, and collapses browser diagnostics, device facts, recovery, and logs until requested | Android Chrome with USB OTG remains physically unverified; hardware access is experimental. |
