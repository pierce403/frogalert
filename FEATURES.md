...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; physical-position `0.2.0-beta.9` source **BLOCKED** for hardware | The physical bottom button rotates uploaded names with a separately rendered count view and has no pairing side effect on either exact profile. The physical top button cycles normal → Bluetooth download → recoverable screen off → normal. Passive scans and temporary alerts run in either visible view, then the selected view is restored | Beta.8 incorrectly standardized logical KEY numbers, so the top image routed its physical top button to the view. Retest both beta.9 images: bottom-only view changes, top-only download/off/wake, no USB-assisted recovery, and long-KEY2 ISP from screen off. |
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; physical-position `0.2.0-beta.9` source **BLOCKED** for new-hash hardware test | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. The physical top button alone shows the Bluetooth animation and enables persistent advertising. The physical bottom button only changes the name/count view and never advertises. Active bitmap streaming retains display ownership | Button roles are compile-time board mappings, not runtime compatibility guesses: `260404` uses KEY2 system/KEY1 view; `250901` uses KEY1 system/KEY2 view. Verify upload/streaming, disconnect, view restoration, screen-off radio silence, and survey resumption on both. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.9` and compact `v0.2.0b9`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.9 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app still uploads normally before marking it hardware-verified. |
