...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; `0.2.0-beta.13` source **BLOCKED** for exact-hash hardware | The physical bottom button rotates uploaded names with a separately rendered count view and has no pairing side effect on either exact profile. The physical top button cycles normal → Bluetooth download → recoverable application screen off → normal. Passive scans and temporary alerts run in either visible view, then the selected view is restored | Beta.13 restores beta.11's screen-off runtime after beta.12's hardware-shutdown regression. Retest both exact images: view/brightness, download/off/wake, BadgeMagic upload, and KEY2 ISP. |
| Compatible with the BadgeMagic app legacy upload path | **REGRESSED** on `0.2.0-beta.12`; beta.13 fix **BLOCKED** for exact-hash hardware test | The owner reports Android name upload works on beta.11 and fails on beta.12. The `FEE0/FEE1` service and parser did not change; beta.13 removes beta.12's only firmware runtime delta—the hardware-shutdown/early-wake integration—and restores beta.11's application screen-off path | Reflash beta.13 on the reporting board and capture app discovery, connection, complete name upload, display reload, disconnect, and a second upload across a survey window. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.13` source **BLOCKED** for hardware | Current source restores beta.11's application screen off and retains exact physical-position roles, Meta-pair and Flipper `0x3081`–`0x3083` detection, steady completed counts, compact boot/version cards, and measured battery status. Local counter receipts are top `c60bebfa…7b9e` (200,344 bytes) and bottom `71865ea7…aa01` (200,376 bytes) | Canonical CI publishes the standard top/bottom pair after build, audit, attestation, quarantine, and provenance checks. Keep `hardware_verified: false` until both exact images pass recovery, USB/App, display, and survey tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, physical-position button roles, boot/version/battery cards, recoverable application screen off, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Beta.13 receipts are top `5af09831…949e` (200,416 bytes) and bottom `5f35682b…5ed3` (200,448 bytes) | The standard publication path does not release this optional lane; physically test both candidates, including presses, brightness, app connection, alerts, screen off/wake, and KEY2 recovery. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.13` and compact `v0.2.0b13`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Physical button behavior with safe screen off | `0.2.0-beta.13` source **BLOCKED** for exact-hash hardware test | Bottom short rotates name/count and bottom long changes brightness; top short cycles `NORMAL → DOWNLOAD → application screen off → NORMAL`. Screen off stops advertising/discovery, TMR0, and matrix drive while retaining button/TMOS/USB/KEY2 recovery tasks | Recheck both exact profiles for button positions, name upload, wake without USB, and continuous roughly 2.2-second KEY2-to-ISP. |
| Brightness and power controls | **PROTOTYPE** in beta.13 | Physical-bottom brightness timing is unchanged. Beta.13 withdraws beta.12's unverified CH582 shutdown integration and restores beta.11's recoverable application screen off | Verify brightness boundaries and normal/download/screen-off current. The application screen-off state is known to draw more than hardware shutdown; lower-current redesign remains blocked on hardware evidence and BadgeMagic compatibility. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.13 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app uploads normally before marking it hardware-verified. |
