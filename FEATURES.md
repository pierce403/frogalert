...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.3` source **BLOCKED** for hardware | The published legacy hashes retain the confirmed detector/display behavior. Current source adds debounced KEY1 rail detection, adaptive button/wake routing, a one-second visible app cue, Meta-pair and Flipper `0x3081`–`0x3083` service detection, a steady completed-result counter, compact boot/version cards, and measured battery status while retaining the KEY2 ISP symbols | Canonical CI now publishes the standard top/bottom pair automatically after build, audit, attestation, quarantine, and provenance checks. It remains `hardware_verified: false` until correct-profile and deliberately mismatched hardware tests pass. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.3` and compact `v0.2.0b3`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.3 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app still uploads normally before marking it hardware-verified. |
...
| Dedicated mobile-first `/flash/` wizard | **SHIPPED** at static UI/browser-test layer | Exactly one pane is visible: fetch and verify both published profile images, show provenance/hardware status, collect informed consent, guide ISP entry, run immediate read-only info, ask which button worked, then treat that answer as the final activation for matching flash/verify and terminal success. No file input exists anywhere in `/flash/`; profile and marking state are internal. Previously authorized devices are detected automatically; first-time WebUSB permission remains one explicit chooser tap. | Both button answers resolve to same-origin beta firmware. `0416:5020` remains a mode hint, not hardware or firmware proof. Android Chrome with USB OTG remains physically unverified. |
...
| Point-of-action KEY2 entry guide | **SHIPPED** at UI/state-test layer | Detected application mode opens a WCH-only chooser from the explicit **Start watching for ISP** tap before the button hold. The first claimed-interface work is `0xA1` Identify plus `0xA7` Read Config; only afterward does the user report Top or Bottom. The guide suggestion never preselects the profile. | Hot-plug updates and the info-held pause still require a physical Chrome/Android test. Confirmed KEY2 behavior exists on the FOSSASIA image for PCB `B1144C_250901`; exact-artifact recovery evidence is required before setting `hardware_verified: true`, and expert-only C3 entry is not a browser checklist. |
...
| Detect connected normal-mode badge | **PROTOTYPE** at code/test layer | The chooser and authorized-device scan recognize application USB `0416:5020`, keep the wizard on the preparation step, and show the button/dot guide without treating its suggested attempt as the target profile. Application interfaces are not opened or claimed. | Browser USB permission is still required once. Neither button working or an uncertain post-info answer reaches the expert-recovery stop boundary; C3 is not a public checklist. Physical browser test pending. |
...
| Pre-armed immediate update after ISP appears | **SHIPPED** at UI/state-test layer | Opened-board and irreversible consent plus the exact phrase are collected before ISP. `0xA1`/`0xA7` run immediately on claim; the post-info Top/Bottom answer is the final destructive action and starts matching config-reset/flash/verify without another prompt | Device detection alone never erases. Authorized-attach timing, double-tap suppression, and the complete Android phone path still need physical tests. |
...
| Exact typed pre-arm and explicit final button action | **SHIPPED** in UI | `ERASE THIS BADGE` is collected before ISP; after read-only info, the destructive Top/Bottom label binds the exact prepared image and is the final action | Physical usability test pending. |
