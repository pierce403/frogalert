...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; no-shutdown `0.2.0-beta.7` source **BLOCKED** for hardware | Upstream's KEY2 display button rotates uploaded names with a separately rendered count view; KEY1 keeps download mode and brightness, but its next short press returns to normal instead of shutdown. Passive scans and temporary alerts run in either view, then the selected view is restored | Exact beta.6 testing proved the shutdown state could strand the bottom badge until USB power returned. Retest both exact beta.7 images, including download-to-normal return and KEY2-only ISP. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.7` source **BLOCKED** for hardware | The published legacy hashes retain the confirmed detector/display behavior. Current source keeps exact profile-bound KEY1 polarity/wake and upstream logical button roles while preventing entry into unverified shutdown, plus a connection-independent one-second visible app cue, Meta-pair and Flipper `0x3081`–`0x3083` service detection, a steady completed-result counter, compact boot/version cards, and measured battery status | Canonical CI publishes the standard top/bottom pair automatically after build, audit, attestation, quarantine, and provenance checks. It remains `hardware_verified: false` until each exact image passes its matching-board tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, upstream button roles, boot/version/battery cards, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Its visible Bluetooth cue ends after one second independently of connection state while the app window remains open for ten. Current images are 200,552 bytes | `0.2.0-beta.7` pre-CI local receipts are top/`260404` `55b5cb13…c81345a0` and bottom/`250901` `4380b3a2…62419203`. The standard publication path does not release this optional lane; physically test both ignored local candidates, including repeated presses, app connection, alerts, power draw, and KEY2 recovery. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.7` and compact `v0.2.0b7`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Upstream button behavior with safe return | Published `0.2.0-beta.6` behavior **REGRESSED** on the bottom badge; corrected source **BLOCKED** for retest | Both profiles keep FOSSASIA's logical roles: KEY1 short enters download and KEY1 long changes brightness; KEY2 short rotates name/count and KEY2 long retains the 200 ms, more-than-ten-sample ISP poll. A second KEY1 short now returns `DOWNLOAD → NORMAL` instead of entering `POWER_OFF` | Beta.6 reproduced the upstream third mode, but its KEY1-only shutdown wake did not restore the bottom badge and the stopped application could no longer process KEY2 recovery. Test repeated system-button taps, advertising closure, return to the selected nametag flow, and a continuous roughly 2.2-second KEY2 hold while the application is running. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.7 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app still uploads normally before marking it hardware-verified. |
