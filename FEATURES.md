...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; physical-position `0.2.0-beta.10` source **BLOCKED** for hardware | The physical bottom button rotates uploaded names with a separately rendered count view and has no pairing side effect on either exact profile. The physical top button cycles normal → Bluetooth download → recoverable screen off → normal. Passive scans and temporary alerts run in either visible view, then the selected view is restored | Beta.9 omitted the profile-constant header from the button module, so its bottom image silently compiled top-profile KEY1 polarity. Retest both beta.10 images: bottom-only view changes, top-only download/off/wake, no USB-assisted recovery, and long-KEY2 ISP from screen off. |
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; physical-position `0.2.0-beta.10` source **BLOCKED** for new-hash hardware test | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. The physical top button alone shows the Bluetooth animation and enables persistent advertising. The physical bottom button only changes the name/count view and never advertises. Active bitmap streaming retains display ownership | Button roles are compile-time board mappings, not runtime compatibility guesses: `260404` uses KEY2 system/KEY1 view; `250901` uses KEY1 system/KEY2 view. The button header now imports those profile constants and fails compilation for a missing or unsupported profile. Verify upload/streaming, disconnect, view restoration, screen-off radio silence, and survey resumption on both. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.10` source **BLOCKED** for hardware | The published legacy hashes retain the confirmed detector/display behavior. Current source keeps exact profile-bound KEY1 polarity and physical button-position roles, uses a recoverable application screen-off state instead of CH58x shutdown, and retains Meta-pair and Flipper `0x3081`–`0x3083` detection, steady completed counts, compact boot/version cards, and measured battery status | Canonical CI publishes the standard top/bottom pair automatically after build, audit, attestation, quarantine, and provenance checks. It remains `hardware_verified: false` until each exact image passes its matching-board tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, physical-position button roles, boot/version/battery cards, recoverable screen off, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Current images are top 200,412 bytes and bottom 200,420 bytes | `0.2.0-beta.10` pre-CI local receipts are top/`260404` `89839dea…0ccf696` and bottom/`250901` `9bcf2013…265f7b0`. The standard publication path does not release this optional lane; physically test both ignored local candidates, including repeated presses, app connection, alerts, screen-off current draw, wake, and KEY2 recovery. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.10` and compact `v0.2.0b10`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Physical button behavior with safe screen off | `0.2.0-beta.10` source **BLOCKED** for exact-hash hardware test | Both profiles expose the same positions: bottom short rotates name/count only; top short cycles `NORMAL → DOWNLOAD → screen off → NORMAL`. The screen-off state disables advertising and surveys, clears the framebuffer, disables the 16 kHz TMR0 display interrupt/timer, and releases all matrix pins, but deliberately leaves TMR3 button scanning, TMOS, USB, and the 200 ms long-KEY2 ISP poll alive. `260404` compiles KEY1 view/KEY2 system; `250901` compiles KEY2 view/KEY1 system | Beta.9's bottom button module could not see the profile constants and compiled the wrong KEY1 polarity. Beta.6's true CH58x shutdown stranded the bottom badge. Test repeated position-based presses, at least one-minute screen off, top-button wake without USB, screen-off current draw, and a continuous roughly 2.2-second KEY2 hold from screen off on both exact boards. |
| Brightness and power controls | **PROTOTYPE** in beta.10 | KEY1 long retains FOSSASIA brightness. Top/`260404` keeps the 25-sample threshold (about 0.5 seconds); bottom/`250901` uses 125 samples (about 2.5 seconds) for its physical top/KEY1 button. Physical top short reaches a recoverable display/radio-off state that can wake without USB | Releasing bottom KEY1 before the long threshold remains an upstream-style short press. Measure current in normal, download, and screen-off states; verify that a completed long KEY1 press never emits its short action on either profile. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.10 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app still uploads normally before marking it hardware-verified. |
