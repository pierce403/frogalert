...
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; either-button attention window **BLOCKED** by a hardware-observed restoration race | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. Either short button now shows the Bluetooth animation and makes the badge discoverable, so app access does not depend on the profile-specific download button and unattended badges remain quiet. The intended visible cue is one second while the radio window stays open for roughly ten; the system button retains ordinary download mode | On `0.2.0-beta.3` top firmware, a bottom-button press sometimes leaves the Bluetooth cue visible instead of restoring the counter. Source inspection found that the cue timeout skips restoration and leaves `app_cue_active` set whenever the peripheral is connected or its state read fails; existing tests assert source shape but do not model that state transition. Reproduce with and without a nearby BadgeMagic connection, fix the timeout/connection ownership contract, then verify upload, disconnect, view restoration, survey resumption, and deliberate cross-profile images. |
