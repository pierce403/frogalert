...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; `0.2.0-beta.14` source **BLOCKED** for exact-hash hardware | The physical bottom button rotates uploaded names with a separately rendered count view and has no pairing side effect on either exact profile. The physical top button cycles normal → Bluetooth download → recoverable application screen off → normal. Passive scans and temporary alerts run in either visible view, then the selected view is restored | Beta.14 restores beta.11's screen-off runtime after beta.12's hardware-shutdown regression. Retest both exact images: view/brightness, download/off/wake, BadgeMagic upload, and KEY2 ISP. |
| Compatible with the BadgeMagic app legacy upload path | **REGRESSED** on `0.2.0-beta.12`; beta.14 fix **BLOCKED** for exact-hash hardware test | The owner reports Android name upload works on beta.11 and fails on beta.12. The `FEE0/FEE1` service and parser did not change; beta.14 removes beta.12's only firmware runtime delta—the hardware-shutdown/early-wake integration—and restores beta.11's application screen-off path | Reflash beta.14 on the reporting board and capture app discovery, connection, complete name upload, display reload, disconnect, and a second upload across a survey window. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.14` source **BLOCKED** for hardware | Current source restores beta.11's application screen off and retains exact physical-position roles, Meta-pair and Flipper `0x3081`–`0x3083` detection, steady completed counts, compact boot/version cards, and measured battery status. Local counter receipts are top `c46504ff…696f` (200,344 bytes) and bottom `e4ff5103…fcda` (200,376 bytes) | Canonical CI publishes the standard top/bottom pair after build, audit, attestation, quarantine, and provenance checks. Keep `hardware_verified: false` until both exact images pass recovery, USB/App, display, and survey tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, physical-position button roles, boot/version/battery cards, recoverable application screen off, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Beta.14 receipts are top `7ec82323…4f2c` (200,416 bytes) and bottom `869ca999…cb65` (200,448 bytes) | The standard publication path does not release this optional lane; physically test both candidates, including presses, brightness, app connection, alerts, screen off/wake, and KEY2 recovery. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.14 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app uploads normally before marking it hardware-verified. |
