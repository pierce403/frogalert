...
| Commit-bound CI candidate bundle | **PROTOTYPE** in workflow, locally verified | After the repository contract passes, a `main` commit touching active firmware inputs builds the locked survey lane and packages a deterministic `0.0.0-candidate.<12-char-commit>` BIN/ELF/checksum/JSON bundle as `frogalert-candidate-<full-commit>` | The Actions artifact expires after 30 days and declares `hardware_verified`, `flash_approved`, `publishable`, and `hosted_on_site` false. It has read-only repository permission, stays outside the manifest/GitHub Release/Pages pipeline, and still needs its first live CI run. |
