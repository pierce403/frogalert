...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; upstream-role `0.2.0-beta.6` source **BLOCKED** for hardware | Upstream's KEY2 display button rotates uploaded names with a separately rendered count view; KEY1 keeps upstream system/power and brightness behavior. Passive scans and temporary alerts run in either view, then the selected view is restored | The prior profile-dependent physical-position routing produced inconsistent bottom-badge behavior. Retest both exact beta.6 images without cross-profile compatibility logic. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.6` and compact `v0.2.0b6`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Upstream button behavior | Published `0.2.0-beta.5` behavior **REGRESSED** on the bottom badge; simplified source **BLOCKED** for retest | Both profiles now keep FOSSASIA's logical roles: KEY1 short advances system/download/power, KEY1 long changes brightness, KEY2 short rotates name/count, and KEY2 long retains ISP. Only exact profile-bound KEY1 polarity and shutdown wake differ | Beta.5's profile-dependent wrappers produced download, brightness, and counter results that did not reliably match physical-position assumptions. Test the electrical roles directly on both exact beta.6 images; do not add cross-flash role adaptation. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.6 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app still uploads normally before marking it hardware-verified. |
