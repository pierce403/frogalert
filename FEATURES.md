...
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; either-button attention window **BLOCKED** for hardware | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. Either short button now shows the Bluetooth animation and makes the badge discoverable, so app access does not depend on the profile-specific download button and unattended badges remain quiet. The visible cue ends after one second while the radio window stays open for roughly ten; the system button retains ordinary download mode | Reflash both candidates and verify the identical one-second cue, Android discovery/connect/upload/disconnect, view restoration, survey resumption, and deliberate cross-profile images in a multi-badge room. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.1` source **BLOCKED** for hardware | The published hashes retain the confirmed detector/display behavior. Current source builds audited 200,836-byte profile candidates with debounced KEY1 rail detection, adaptive button/wake routing, a one-second visible app cue, Meta-pair detection, steady completed-result counter, compact boot/version cards, and measured battery status while retaining the KEY2 ISP symbols | Pre-CI local receipts are top/`260404` `cd546bc1…e4700022` and bottom/`250901` `054c6ebd…2d220621`. GitHub must reproduce and attest the exact candidate, then correct-profile and deliberately mismatched hardware tests must pass before replacing the beta. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, adaptive buttons, boot/version/battery cards, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Its visible Bluetooth cue ends after one second while the app window remains open for ten. Current images are 200,920 bytes | Pre-CI local receipts are top/`260404` `76a43c83…95b23bda` and bottom/`250901` `ef6da5d1…6619bdd2`. Verify GitHub reproduces them, then test both boards, including repeated presses, app connection, alerts, power draw, and KEY2 recovery. |
...
| Reproducible release build | **SHIPPED** as a cloud candidate pipeline; release still hardware-gated | GitHub Actions builds both profiles and lanes from pinned source/toolchain inputs, retains all structural/ELF/vector/profile audits, records calculated output hashes, attests the binaries, and uploads a 90-day commit-bound artifact. Manual workflow dispatch supports phone-only development | Exact bytes still need profile-bound physical evidence before the approval commit can publish them. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.1` and compact `v0.2.0b1`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | Hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value on each profile before release. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.1 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built candidate and confirm the Android app still uploads normally before release. |
...
| Unagi and observed Meta seeds | **SHIPPED** at core layer, **PROTOTYPE** in survey build | Flipper, Axon Body, TASER, and Ray-Ban variants are mirrored in bounded C; Meta glasses additionally require both passive `0x01AB` and `0xFD5F` fields | The Rust ABI remains a separate gate despite matching behavior. |
...
| Commit-bound CI candidate bundle | Existing single-lane bundle live-CI verified; dual-lane schema-3 path **PROTOTYPE** pending first run | Candidate CI builds both profiles for counter and `frogs`, packages separate directories under `frogalert-candidate-<full-commit>`, records version/run/attempt and exact hashes, attests BINs/ELFs, and keeps every approval flag false | Both directories remain hardware-unverified Actions evidence outside the manifest, GitHub Releases, and Pages executable catalog. The public front page continues to identify the latest physically approved release. |
...
| Requirement | Status | Acceptance evidence | Dependency / notes |
| --- | --- | --- | --- |
| Versioned FrogAlert raw `.bin` | **AVAILABLE** as beta | Exact `0.1.0-beta.1` top-button (`260404`) and bottom-button (`250901`) BINs remain same-origin legacy release assets. New versions are built in Actions and materialized from one recorded candidate only after approval | Unknown PCB revisions remain refused; new generated BINs are not committed. |
...
| Machine-readable manifest | **SHIPPED** for beta and cloud promotion | Schema v5 lists both exact USB-C beta releases, separates labs/recovery, pins publication dates, and requires run/artifact/digest/candidate-receipt provenance for every non-legacy version | Every published version must contain exactly one top/`260404` and one bottom/`250901` image; a partial pair fails site assembly and browser catalog loading. |
...
| Build provenance | **SHIPPED** at source/test layer; first new live run pending | Candidate metadata records version, source, GitHub run/workflow/job/attempt, profiles, lane, toolchain, and exact BIN/ELF hashes; Actions attests outputs. A separate trusted CI job materializes approved bytes, while Pages requires the canonical `.github/workflows/ci.yml` trigger path and proves the triggering SHA is still current `main` before checkout | Provenance proves where bytes came from, not that they are safe on hardware. |
...
| Commit-driven GitHub release automation | **VERIFIED** for the legacy beta; cloud-artifact promotion **PROTOTYPE** | CI run `30427180021` and release/Pages run `30427231242` published `v0.1.0-beta.1`. Current code additionally materializes future approved bytes from one provenance-bound Actions candidate and feeds the same staged bytes to GitHub Releases and Pages | Run the first schema-5 cloud candidate through physical approval and public publication before calling the new materialization path production-verified. |
...
| Clear latest-firmware state | **SHIPPED** | The homepage reads the validated manifest, labels the newest approved semantic version/channel/date, and lists its exact supported CH582M PCB markings | An untested source version remains an Actions candidate and never replaces the public latest release. |
...
| Private phone-only candidate test lab | **PLANNED**, design recorded | A Codespaces-only generated route would download one authenticated Actions candidate server-side, verify canonical run/artifact/receipt/hash/attestation gates, and reuse the explicit WebUSB program/verify flow without placing unverified bytes in Pages or the public catalog | First prove a private top-level `*.app.github.dev` forwarded URL exposes `navigator.usb` on Android Chrome and does not send `Permissions-Policy: usb=()`. Static Pages cannot safely fetch private Actions artifacts without exposing credentials. |
...
| Same-origin release manifest | **SHIPPED** for beta | Schema v5 lists two exact-profile `0.1.0-beta.1` releases plus one recovery descriptor; release options use stable ids and same-origin BINs | Site assembly requires exact hashes and evidence records. New versions additionally require cloud candidate provenance; schema-2 beta evidence and schema-1 stable evidence retain their existing gates. |
...
| SHA-256 calculation | **SHIPPED** for approved artifacts | Web Crypto hashes every fetched image and requires the manifest SHA-256 before destructive arming; the same gate covers locally derived inspection artifacts | Physical WebUSB program/verify evidence remains separate from browser-side download integrity. |
...
| Released FrogAlert firmware one-click selection | **SHIPPED** for `0.1.0-beta.1` | The profile-aware wizard automatically selects and verifies the published top or bottom image after observing the matching button path; no file chooser is present | Android Chrome/WebUSB physical flashing remains unverified, and unapproved Actions candidates stay unavailable here. |
