...
| Build provenance | **SHIPPED** through live candidate creation | Run `30827723476` recorded version, source, GitHub run/workflow/job/attempt, profiles, lane, toolchain, and exact BIN/ELF hashes; `attest-candidate` and the separate trusted `publication-assets` job passed. Pages run `30827863178` proved the canonical workflow/current-main gate and deployed only the still-approved beta catalog | Provenance proves where bytes came from, not that they are safe on hardware. The first schema-5 physical approval/promotion is still pending. |
