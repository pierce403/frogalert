...
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for hardware | A distinct `frogs` lane retains passive detection, fixed alerts, BadgeMagic uploads, adaptive buttons, and KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Locked images are 205,972 bytes | Top (`260404`) SHA is `115ac296…c838f50`; bottom (`250901`) SHA is `49592198…57a38`. Verify both exact boards, deliberate cross-profile flashes, alert/app preemption and restoration, power draw, and KEY2-only recovery before release. |
