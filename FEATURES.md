...
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; always-discoverable candidate **BLOCKED** for hardware | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. Current source additionally advertises in normal nametag mode so app access does not depend on the profile-specific download button | Reflash both candidates and verify Android discovery, connect, upload, disconnect, survey resumption, and a deliberate cross-profile image. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; newer adaptive candidate **BLOCKED** for hardware | The published hashes retain the confirmed detector/display behavior. Current source additionally builds audited 205,596-byte profile candidates with debounced KEY1 rail detection, adaptive button/wake routing, and normal-mode BadgeMagic advertising while retaining the KEY2 ISP symbols | The adaptive `260404` SHA is `52333c72…6f7cf6`; the adaptive `250901` SHA is `90f6c7cd…57386`. Do not replace the beta until correct-profile and deliberately mismatched hardware tests pass. |
...
| Peripheral/observer role switching | **PROTOTYPE** in source, **BLOCKED** for hardware | WCH Central and Peripheral roles initialize together. The candidate advertises the BadgeMagic service in normal nametag mode, pauses advertising before discovery rather than scanning concurrently, and suspends surveys after a connection | Repeated 24-hour run plus Android discovery/upload/reconnect tests are required on both exact boards and cross-profile flashes. |
