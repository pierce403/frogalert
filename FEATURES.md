...
| FOSSASIA-shell passive survey candidate | **PROTOTYPE** at local build layer, **PARTIAL** on hardware | The preceding candidate displayed its `BT 00` placeholder on the photographed badge but did not visibly advance to a measured result. The replacement is a locked 201,788-byte BIN SHA-256 `9d35de6a…2c51a7`; passive start/cancel/suspend symbols, bounded OUI/name/service classifier, exact view rotation, alert/frog renderers, qualified 48-column frame cropping, live-result and completion-list paths, live vectors, retained USB/BLE/display/KEY2 symbols, no AMO/LR/SC, ELF/BIN identity, and 9,788 bytes RAM headroom pass | Private `tmp/` diagnostic only. Short KEY2 rotates names with the counter while KEY1 behavior is unchanged; surveys run in both views. Discovery cancellation waits for WCH's completion event before advertising or connecting resumes, and an interrupted app stream is cleared on disconnect. Three-second `COP DETECTED`/`FLIPPER DETECTED`/`KARR DETECTED` and frog overlays restore the selected view. A continuously present match can retrigger in each roughly 20-second survey window. It still needs a hash-bound physical flash and smoke test. |
...
| Reproducible release build | **PROTOTYPE** for baseline/canary/survey | Independent clean baseline builds reproduce `2049eb58…f670a2`; canary builds reproduce `6591f55f…03e87`; survey builds reproduce `9d35de6a…2c51a7` | A release build still needs a source commit, clean CI receipt, and physical evidence. |
| Firmware size limit | **SHIPPED** at build layer | Binary audit rejects images over 448 KiB; survey is 201,788 bytes | Keep the exact limit in the pinned profile and release checks. |
...
| Flipper alert text | **SHIPPED** at host layer, **PROTOTYPE** in survey build | Rust policy returns `FLIPPER DETECTED`; the C-shell diagnostic safely parses legacy/extended complete and shortened local-name fields and renders the same 16-character message for three seconds | Unagi uses a name rule, not an OUI. Official firmware advertises `xFlipper <name>`; renamed/custom devices can evade it and other devices can spoof it. The embedded C mirror is temporary until the Rust ABI canary passes. |
| Cop alert text | **SHIPPED** at host layer, **PROTOTYPE** in survey build | The bounded C mirror maps both documented public-address OUIs plus `Axon Body`, `TASER`, `Ray-Ban`, and `Ray Ban` name hints to a three-second `COP DETECTED` overlay | OUI matching is restricted to controller-reported public addresses. Names and vendor prefixes remain spoofable hints, not identity proof. |
| KARR alert text | **SHIPPED** at host layer, **PROTOTYPE** in survey build | Rust and the bounded C mirror require case-insensitive `QT ` at the start of the advertised name plus a non-empty serial value, then render `KARR DETECTED` for three seconds | This user-observed name prefix is spoofable and can miss devices whose name appears only in a scan response. Hardware behavior remains unverified. |
| BadgeMagic frog animation | **PROTOTYPE** in survey build, **BLOCKED** for hardware | An exact case-insensitive `LED Badge Magic` local name or advertised `0xFEE0` service renders three frogs in two alternating frames for three seconds | Passive reports may omit a scan-response-only name. The `0xFEE0` fallback can false-positive compatible devices that reuse the service UUID. |
| User framebuffer storage | **PLANNED** | Upload survives alert and reboot | Define data-flash ownership/versioning. |
| Temporary alert overlay | **PROTOTYPE** in survey build, **BLOCKED** for hardware | Cop/Flipper/KARR and frog overlays expire after three seconds and restore the selected nametag or count view without writing the uploaded payload | Prove restoration after each rule, view transition, streaming session, and power cycle. |
...
| Pinned FOSSASIA USB-C build helper | **SHIPPED** at local build layer | Exact source/toolchain/tree hashes, `USBC_VERSION=1`, byte-identical baseline, locked canary/survey, ELF-to-BIN identity, runtime/USB/vector/symbol/instruction/RAM audits, fail-closed artifact cleanup, and ignored `tmp/` output | First run downloads about 345 MB. It never flashes, publishes, or authorizes a test. |
| Commit-bound CI candidate bundle | **PROTOTYPE**, live-CI verified | After the repository contract passes, a `main` commit touching active firmware inputs builds the locked survey lane and packages a deterministic `0.0.0-candidate.<12-char-commit>` BIN/ELF/checksum/JSON bundle as `frogalert-candidate-<full-commit>` | First live run `30069161224` passed for commit `af83fbb`; the downloaded 201,788-byte BIN matched `9d35de6a…c51a7` and both bundle checksums passed. The Actions artifact expires after 30 days, declares every publish/approval flag false, has read-only repository permission, and stays outside the manifest/GitHub Release/Pages pipeline. |
...
| Point-of-action KEY2 entry guide | **SHIPPED** at UI/state-test layer | Inline compatible-firmware-only five-step flow, back/retry states, center-dot acknowledgement, observed 9–13 second window, and original/unknown-firmware stop boundary sit beside the chooser | Confirmed on the FOSSASIA image for PCB `B1144C_250901`; every FrogAlert release still needs exact-artifact recovery evidence, and expert-only C3 entry is not a browser checklist. |
...
- **PROTOTYPE at build layer / PARTIAL on hardware:** the preceding private
  ...
  OUI/name rule in bounded C; renders three-second cop/Flipper and BadgeMagic
  frog overlays on a roughly 20-second survey cadence; restores the selected view and
