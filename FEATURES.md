...
| Nearby-device count rendering | **SHIPPED** at host layer, **PARTIAL** on hardware | Rust host renderer tests count/saturation; the preceding FOSSASIA-shell diagnostic visibly scrolled `BT 00` on the photographed badge. The replacement keeps `BT 00` through `BT 64+` as one centered fixed frame and appends `I`, `R`, `S`, `E`, or `T` while that phase is active | The user report proves the older display hook, not the new fixed-frame candidate or a completed survey. Nametag view remains selected between alerts instead of being permanently masked. |
| Flipper alert text | **SHIPPED** at host layer, **PROTOTYPE** in survey build | Rust policy returns `FLIPPER DETECTED`; the C-shell diagnostic safely parses legacy/extended complete and shortened local-name fields and renders `FLIPPER`, then `DETECTED`, as two fixed 1.5-second pages | Unagi uses a name rule, not an OUI. Official firmware advertises `xFlipper <name>`; renamed/custom devices can evade it and other devices can spoof it. The embedded C mirror is temporary until the Rust ABI canary passes. |
| Cop alert text | **SHIPPED** at host layer, **PROTOTYPE** in survey build | The bounded C mirror maps both documented public-address OUIs plus `Axon Body`, `TASER`, `Ray-Ban`, and `Ray Ban` name hints to fixed `COP`, then `DETECTED`, pages over three seconds | OUI matching is restricted to controller-reported public addresses. Names and vendor prefixes remain spoofable hints, not identity proof. |
| KARR alert text | **SHIPPED** at host layer, **PROTOTYPE** in survey build | Rust and the bounded C mirror require case-insensitive `QT ` at the start of the advertised name plus a non-empty serial value, then render fixed `KARR`, then `DETECTED`, pages over three seconds | This user-observed name prefix is spoofable and can miss devices whose name appears only in a scan response. Hardware behavior remains unverified. |
...
| Temporary alert overlay | **PROTOTYPE** in survey build, **BLOCKED** for hardware | Built-in/custom text uses no more than two fixed 1.5-second pages; frog overlays retain two deliberate poses. Both expire after three seconds, prevent queued name animations from taking ownership, and restore the selected nametag/count view without writing the uploaded payload | Prove page readability and restoration after each rule, view transition, streaming session, and power cycle. |
