...
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** on both beta profiles; either-button attention window **BLOCKED** for hardware | Survey keeps FOSSASIA `FEE0/FEE1` code and skips connected states; the user confirmed uploads on the released firmware line. Either short button now pauses surveying and makes the badge discoverable for roughly ten seconds, so app access does not depend on the profile-specific download button and unattended badges remain quiet | Reflash both candidates and verify Android discovery, connect, upload, disconnect, survey resumption, and deliberate cross-profile images in a multi-badge room. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; newer adaptive candidate **BLOCKED** for hardware | The published hashes retain the confirmed detector/display behavior. Current source additionally builds audited 205,852-byte profile candidates with debounced KEY1 rail detection, adaptive button/wake routing, and an expiring either-button app-attention window while retaining the KEY2 ISP symbols | The adaptive `260404` SHA is `d59e8cc2…698614`; the adaptive `250901` SHA is `532a6628…fe295c1`. Do not replace the beta until correct-profile and deliberately mismatched hardware tests pass. |
