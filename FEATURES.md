...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; `0.3.0-beta.1` source **BLOCKED** for exact-hash hardware | The physical bottom button rotates uploaded names with a separately rendered count view and has no pairing side effect on either exact profile. The physical top button cycles normal → Bluetooth download → hardware shutdown; exact-profile wake cold-boots normal. Passive scans and temporary alerts run in either visible view, then the selected view is restored | The Rust reducer adds confirmed cancellation and advertising/connection shutdown handoff while retaining button-only wake. Retest both exact images: view/brightness, download/off/wake, prolonged darkness, BadgeMagic upload, current, and KEY2 ISP. |
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** by owner on beta.12; Rust application **BLOCKED** for exact-hash hardware test | The owner now attributes the earlier beta.12 failure to a noisy Bluetooth environment. Current source preserves 16-byte `FEE1` writes with responses while fixing the accepted connection's parameter request, extending supervision to six seconds, allocating the padded final packet, bounding declared data, checking flash status, and clearing partial state on disconnect | Test complete and interrupted/retried Android uploads in quiet and congested environments on both profiles; confirm display reload, disconnect, and unattended advertising state. |
| Detection is passive and local | **SHIPPED** at core layer, **PROTOTYPE** in survey build | Survey calls passive WCH discovery, feeds the shared Rust classifier, and has no network/storage path | The firmware and emulator run the same Rust application; host evidence does not certify physical radio behavior. |
...
| Automatic USB-C profile detection | **REJECTED** | Untouched KEY1 is open on both boards, and a physical `250901` test showed the held-KEY1 weak-pull probe could falsely classify open PA1 as `260404`, swapping the short-button roles. Current source keeps the artifact's compiled KEY1 polarity and physical-position routing | Require the exact printed marking and matching profile for every flash. Deliberate cross-profile images remain useful only for proving fail-safe recovery boundaries; firmware must not silently reinterpret their hardware profile. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.3.0-beta.1` source **BLOCKED** for hardware | Current source uses radio-idle CH582 shutdown, button-only qualified wake, and the hardened legacy transfer receiver while retaining physical-position roles, Meta-pair and Flipper `0x3081`–`0x3083` detection, steady counts, boot/version cards, and measured battery status. The Rust application and SDK emulation replace the C policy/state implementations; exact hashes come from canonical candidate receipts | Canonical CI publishes the standard top/bottom pair after build, audit, attestation, quarantine, and provenance checks. Keep `hardware_verified: false` until both exact images pass recovery, USB/App, display, current, prolonged-off, and survey tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, hardened BadgeMagic uploads, physical-position button roles, boot/version/battery cards, button-only hardware shutdown wake, and early KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. All four profile/view builds pass local structural audits; canonical receipts supply exact hashes | The standard publication path does not release this optional lane; physically test both candidates, including presses, brightness, app connection/retry, alerts, prolonged screen off, current, wake, and KEY2 recovery. |
| Rust for embedded application logic | **SHIPPED** at host/build layer; hardware unverified | The shipping `no_std` application owns scan/shutdown state, detection/configuration, counting, rendering, boot/battery cards, and upload validation. Firmware and SDK emulator use the same core | C retains startup, drivers, interrupts, allocation/flash, nametag storage, buttons and ROM ISP; the standalone Rust runtime remains quarantined. |
| Atomic-free Rust archive | **SHIPPED** at build layer | Rust 1.98.1 targets IMC with atomics disabled; all four final linked images pass instruction/vector/RAM/ELF binding audits | LLVM objcopy removes only non-loadable `.riscv.attributes` for the pinned MRS linker. Compiler commit and library SHA are recorded in candidate receipts. |
...
| Linker/runtime configuration | **FAILED** for standalone Rust; FOSSASIA replacement **SHIPPED** at build layer | Linked ELF proves Timer 0 vector 16 contained `DefaultInterruptHandler` because PAC 0.3 put `__EXTERNAL_INTERRUPTS` in flash instead of the runtime's RAM vector section | Replacement images inherit FOSSASIA startup/linker/runtime unchanged; a post-link vector audit now guards any future runtime work. |
...
| Panic/fault behavior | **PROTOTYPE** | Rust cannot unwind across C; embedded panic calls the retained SDK reset routine | The FOSSASIA shell owns early recovery; physically force and observe a fault before claiming recovery evidence. |
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.3.0-beta.1` and compact `v0.3.0b1`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Physical button behavior with safe screen off | `0.3.0-beta.1` source **BLOCKED** for exact-hash hardware test | Bottom short rotates name/count and bottom long changes brightness; top short cycles `NORMAL → DOWNLOAD → hardware shutdown`, with wake cold-booting normal. Radio idle is confirmed before shutdown; only exact-profile buttons can wake; unqualified early wake returns to shutdown | Recheck both exact profiles for button positions, name upload, wake without USB, prolonged darkness across charger-state changes, and continuous roughly 2.2-second KEY2-to-ISP. |
| Brightness and power controls | **PROTOTYPE** in beta.16 | Physical-bottom brightness timing is unchanged. Charger status remains readable during normal operation but can no longer wake screen off; explicit buttons retain wake and recovery | Verify brightness boundaries, normal/download/screen-off current, repeated multi-hour dark cycles plugged and unplugged, and KEY2 recovery on both profiles. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.3.0-beta.1 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app uploads normally before marking it hardware-verified. |
...
| Public-address OUI matching | **SHIPPED** at core layer, **PROTOTYPE** in survey build | Axon/Flock unit tests and Rust firmware use controller-reported address type | Physical reports must confirm WCH byte order and address-type mapping on the exact image. |
...
| Unagi and observed Meta seeds | **SHIPPED** at core layer, **PROTOTYPE** in survey build | Flipper names and serial-profile services `0x3081`–`0x3083`, Axon Body, TASER, and Ray-Ban variants run in the shared Rust classifier; Meta glasses additionally require both passive `0x01AB` and `0xFD5F` fields | The C ABI is compiled and checked for both profiles; exact-image physical testing remains pending. |
...
| Parse BLE advertisement fields | **SHIPPED** at host layer, **PROTOTYPE** in survey build | Complete/shortened-name and malformed-length tests pass in The shared Rust application | [`advertisement.rs`](crates/frogalert-core/src/advertisement.rs) is allocation-free; the firmware invokes this same parser through the tested primitive C ABI. |
...
| Peripheral/observer role switching | **PROTOTYPE** in source, **BLOCKED** for hardware | WCH Central and Peripheral roles initialize together. The profile-mapped physical top/system button suspends discovery before enabling persistent advertising; the physical bottom/view button has no radio side effect. Screen off disables both advertising and surveys. Otherwise advertising is paused before each survey rather than scanning concurrently, and a connection suspends surveys | Repeated 24-hour run plus Android discovery/upload/reconnect tests are required on both exact boards. |
...
| Pinned FOSSASIA USB-C build helper | **SHIPPED** at local build layer | Exact source/toolchain/tree hashes, default `260404` and explicit legacy `250901` profiles, common `USBC_VERSION=1` matrix, exact profile-bound KEY1 patch, profile/lane locks, ELF-to-BIN identity, runtime/USB/vector/symbol/instruction/RAM audits, fail-closed cleanup, and explicit `frogalert-top-b1144c-260404` / `frogalert-bottom-b1144c-250901` aliases | First run downloads about 345 MB. It never flashes, publishes, or authorizes a test; exact profile selection remains mandatory. |
...
| USB-C KEY1 wiring/polarity detection | **REJECTED** at runtime | CHIP ID, USB descriptors, and untouched open KEY1 cannot distinguish the boards. The held-button weak-pull probe also produced a false `260404` result on a physical bottom-profile test | The flasher uses the observed profile-specific KEY2 path and refuses to guess. Firmware retains its compiled exact profile; printed marking and the Top/Bottom selection remain authoritative. |
...
- **PROTOTYPE at build layer / PARTIAL on hardware:** the preceding
  ...
  explicit in Rust state while the counter holds only the latest completed
  result; scans in either visible
  view; consumes live reports and the completion list; runs every documented
  OUI/name rule in the shared Rust application; accepts a CRC/profile-bound built-in/custom rule
...
- **SHIPPED at host/build layer:** Rust application and deterministic SDK emulator,
  real C ABI conformance, transfer fault injection, and a 24-hour soak across all
  four profile/view configurations.
- **PLANNED:** refine per-rule cooldowns, measure battery impact, and physically validate the
