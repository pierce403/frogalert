...
| Linux BLE methodology probe | **SHIPPED** at host research layer | BlueZ discovery plus root raw-HCI passive/active comparison; parser tests cover legacy and extended reports, scan responses, malformed AD data, and anonymous labels | Raw HCI can be blocked by another discovery owner. Meta-assigned IDs are research hints; a physical-glasses comparison is still pending. |
