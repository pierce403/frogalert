...
| --- | --- | --- | --- |
| Normal operation remains useful as a nametag | **VERIFIED** on the legacy beta pair; `0.2.0-beta.15` source **BLOCKED** for exact-hash hardware | The physical bottom button rotates uploaded names with a separately rendered count view and has no pairing side effect on either exact profile. The physical top button cycles normal → Bluetooth download → hardware shutdown; exact-profile wake cold-boots normal. Passive scans and temporary alerts run in either visible view, then the selected view is restored | The owner clarified beta.12 low power worked; beta.15 restores it. Retest both exact images: view/brightness, download/off/wake, BadgeMagic upload, current, and KEY2 ISP. |
| Compatible with the BadgeMagic app legacy upload path | **VERIFIED** by owner on beta.12; beta.15 hardening **BLOCKED** for exact-hash hardware test | The owner now attributes the earlier beta.12 failure to a noisy Bluetooth environment. Current source preserves 16-byte `FEE1` writes with responses while fixing the accepted connection's parameter request, extending supervision to six seconds, allocating the padded final packet, bounding declared data, checking flash status, and clearing partial state on disconnect | Test complete and interrupted/retried Android uploads in quiet and congested environments on both profiles; confirm display reload, disconnect, and unattended advertising state. |
...
| FOSSASIA-shell passive survey firmware | **VERIFIED** `0.1.0-beta.1` on both exact profiles; `0.2.0-beta.15` source **BLOCKED** for hardware | Current source restores radio-idle CH582 shutdown and exact-profile wake, hardens the legacy transfer receiver, and retains physical-position roles, Meta-pair and Flipper `0x3081`–`0x3083` detection, steady counts, boot/version cards, and measured battery status. Local counter receipts are top `53b094f7…048d4` (201,224 bytes) and bottom `5c4cd361…f6254` (201,308 bytes) | Canonical CI publishes the standard top/bottom pair after build, audit, attestation, quarantine, and provenance checks. Keep `hardware_verified: false` until both exact images pass recovery, USB/App, display, current, and survey tests. |
| Dancing-frog alternate view | **PROTOTYPE** for both exact profiles, **BLOCKED** for corrected hardware test | A distinct `frogs` lane retains passive detection, fixed alerts, hardened BadgeMagic uploads, physical-position button roles, boot/version/battery cards, hardware shutdown, and early KEY2 recovery while changing only the alternate counter view to three fixed frogs alternating every 500 ms. Local receipts are top `d2e6a51b…afc74` (201,312 bytes) and bottom `54af790b…40293` (201,396 bytes) | The standard publication path does not release this optional lane; physically test both candidates, including presses, brightness, app connection/retry, alerts, screen-off current, wake, and KEY2 recovery. |
...
| Version embedded in firmware | **PROTOTYPE** in current survey/frog source | `firmware/fossasia-usbc/version.json` supplies semantic `0.2.0-beta.15` and compact `v0.2.0b15`; the compact version and compiled top/up or bottom/down marker are shown at boot, while the full version is exposed through Device Information | CI publishes the standard pair after automated validation; hardware-confirm both arrow orientations, 3×5 text readability, and the exact GATT value before marking either artifact hardware-verified. |
...
| Physical button behavior with safe screen off | `0.2.0-beta.15` source **BLOCKED** for exact-hash hardware test | Bottom short rotates name/count and bottom long changes brightness; top short cycles `NORMAL → DOWNLOAD → hardware shutdown`, with wake cold-booting normal. Radio idle is confirmed before shutdown and held KEY2 is qualified before BLE/USB/display startup | Recheck both exact profiles for button positions, name upload, wake without USB, charge wake, and continuous roughly 2.2-second KEY2-to-ISP. |
| Brightness and power controls | **PROTOTYPE** in beta.15 | Physical-bottom brightness timing is unchanged. Beta.15 restores beta.12's lower-power CH582 shutdown after the owner clarified beta.12 worked normally | Verify brightness boundaries, normal/download/screen-off current, charge wake, repeated cycles, and KEY2 recovery on both profiles. |
...
| Device Information version | **PROTOTYPE** in current survey/frog source | `0x180A/0x2A26` now reports `FrogAlert 0.2.0-beta.15 / FOSSASIA v0.1-42-g9ce885d`; manufacturer remains `FOSSASIA` | Read both characteristics from each exact cloud-built release and confirm the Android app uploads normally before marking it hardware-verified. |
