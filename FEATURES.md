...
| Dedicated mobile-first `/flash/` wizard | **SHIPPED** at static UI/browser-test layer | Exactly one pane is visible: connect and read-only identify, choose exact board/BIN, confirm, flash/verify, then terminal success. Previously authorized ISP devices are detected automatically; first-time WebUSB permission remains one explicit chooser tap. The old dashboard is hidden from both display and accessibility trees. | Android Chrome with USB OTG remains physically unverified; hardware access is experimental. |
