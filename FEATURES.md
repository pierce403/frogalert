...
| Stable refresh and display ownership | **PROTOTYPE** in survey source, **BLOCKED** after FrogAlert changes | Survey no longer clears the framebuffer every 100 ms and consumes already queued marquee/flash/fixed/Bluetooth animation events while an overlay owns the panel | This addresses the observed competing-scroll source path; FOSSASIA's roughly 45 Hz multiplex refresh remains unchanged. Repeat visual/current checks on each exact profile/hash and during scan/overlay windows. |
...
| Temporary alert overlay | **PROTOTYPE** in survey build, **BLOCKED** for hardware | Built-in/custom text and frog overlays expire after three seconds; event ownership guards prevent queued scrolling modes from restarting beneath them, then restore the selected nametag/count view without writing the uploaded payload | Prove restoration after each rule, view transition, streaming session, and power cycle. |
...
| Button behavior preserved | **PROTOTYPE** at build layer, **BLOCKED** for hardware | `260404` builds use pull-up/active-low/falling-wake KEY1; `250901` retains pull-down/active-high/rising-wake KEY1. Short KEY2 rotates `Name 1 → BT counter → Name 2 → BT counter`, while long-KEY2 recovery remains intact | Run profile-appropriate KEY1 download/power/brightness/wake, the complete rotation, and long-KEY2 dot-to-ISP checks on each exact hash. |
...
- **PROTOTYPE at build layer / PARTIAL on hardware:** the preceding private
  ...
  block; renders three-second cop/Flipper/custom and BadgeMagic frog overlays
