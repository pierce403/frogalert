...
const OUI_RULES: &[OuiRule] = &[
    OuiRule { prefix: [0x00, 0x25, 0xDF], kind: AlertKind::Cop, label: "Axon OUI" },
    OuiRule { prefix: [0xB4, 0x1E, 0x52], kind: AlertKind::Cop, label: "Flock Safety OUI" },
];

// These mirror Unagi's currently seeded name rules.
const NAME_RULES: &[NameRule] = &[
    NameRule { needle: b"axon body", kind: AlertKind::Cop, label: "Axon name" },
    NameRule { needle: b"taser", kind: AlertKind::Cop, label: "TASER name" },
    NameRule { needle: b"flipper", kind: AlertKind::Hax, label: "Flipper name" },
    NameRule { needle: b"ray-ban", kind: AlertKind::Hax, label: "Ray-Ban name" },
    NameRule { needle: b"ray ban", kind: AlertKind::Hax, label: "Ray Ban name" },
];
    ...
    if observation.public_address {
        for rule in OUI_RULES {
            if starts_with_either_order(&observation.address, &rule.prefix) {
                return Some(Match { kind: rule.kind, label: rule.label });
            }
        }
    }
...
mod tests {
    ...
    fn observation(address: [u8; 6], public_address: bool, name: Option<&[u8]>) -> Observation<'_> {
        Observation { address, public_address, name }
    }
    ...
    fn rejects_oui_on_random_address() {
        assert_eq!(classify(&observation([0x00, 0x25, 0xDF, 1, 2, 3], false, None)), None);
    }
    ...
    fn name_rule_still_works_with_random_address() {
        let found = classify(&observation([0xC2, 0, 0, 0, 0, 1], false, Some(b"Axon Body 4"))).unwrap();
        assert_eq!(found.kind, AlertKind::Cop);
    }

    #[test]
    fn unrelated_device_is_ignored() {
        assert_eq!(classify(&observation([1, 2, 3, 4, 5, 6], true, Some(b"Headphones"))), None);
    }
}
